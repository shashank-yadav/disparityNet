...
def vgg_16(inputs,
  ...
  out = dict()
  with variable_scope.variable_scope(scope, 'vgg_16', [inputs], reuse=tf.AUTO_REUSE) as sc:
    end_points_collection = sc.original_name_scope + '_end_points'
    # Collect outputs for conv2d, fully_connected and max_pool2d.
    with arg_scope(
        ...
        outputs_collections=end_points_collection):
      net = layers_lib.repeat(
          inputs, 2, layers.conv2d, 64, [3, 3], scope='conv1', trainable=is_training)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool1')
      out['conv1']  = net
      
      net = layers_lib.repeat(net, 2, layers.conv2d, 128, [3, 3], scope='conv2', trainable=is_training)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool2')
      out['conv2']  = net

      net = layers_lib.repeat(net, 3, layers.conv2d, 256, [3, 3], scope='conv3', trainable=is_training)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool3')
      out['conv3']  = net

      net = layers_lib.repeat(net, 3, layers.conv2d, 512, [3, 3], scope='conv4', trainable=is_training)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool4')
      out['conv4']  = net

      net = layers_lib.repeat(net, 3, layers.conv2d, 512, [3, 3], scope='conv5', trainable=is_training)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool5')
      out['conv5']  = net
